...
Due to this feature we can use two different adam in order to properly leverage the structure of the proposed SVRPG update. The first adam will be associated to the snapshot updates and the second one will be associated to the sub-iteration updates. By doing so we are decoupling the contribution to the variance introduced by the snapshots from that one introduced by the sub-iterations, which lie, respectively, on two different order of magnitude.Therefore, we are able to properly adapt the learning rate in each situation. 
...
\todobep{also here I tried to solve the issues highlighted}
