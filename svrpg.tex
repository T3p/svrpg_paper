...
In all the experiments we used gaussian policies and bootsrap confidence intervals $\alpha = 10\% $ over ten different runs for evaluation. Each run with a different policy initialization which is kept fixed when changing the algorithm. The maximum number of trajectories at hand is fixed for each task. 
