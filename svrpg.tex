...
\]
\end{restatable}
The resulting SVRPG algorithm is detailed in Algorithm \ref{alg:svrpg}.
...
\begin{algorithm}[tb]
	...
	\begin{algorithmic}
		...
		\STATE $\gradApp{\tilde{\vtheta}^{s}}{N} = \frac{1}{N}\sum_{j=0}^{N-1}\score{}{\tau_j\mid\tilde{\vtheta}^{s}}$
		\STATE $\vtheta_0^{s+1} = \tilde{\vtheta}^s = \vtheta_{m-1}^s$
		\FOR{$t=0$ {\bfseries to} $m-1$}
		\STATE Sample trajectory $\tau_i$ from 				$\pi(\cdot\mid\tilde{\vtheta}^{s})$
		\STATE 
		\begin{align*}
		\blacktriangledown J(\vtheta_t^{s+1}) = 
		&\gradApp{\tilde{\vtheta}^s}{N}
		+ \omega(\tau_i)\score{}{t_i\mid\vtheta_t^{s+1}} \\
		& - \score{}{\tau_i \mid \tilde{\vtheta}^{s+1}}
		\end{align*}
