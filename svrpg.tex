...
Under this hypothesis, full gradient (FG) ascent ~\citep{cauchy1847methode} with a constant step size achieves a convergence rate of $\mathcal{O}(\nicefrac{1}{\epsilon})$, \ie it requires $T = \mathcal{O}(\nicefrac{1}{\epsilon})$ iterations to achieve $\norm[2]{\nabla_{\vtheta}f(\vtheta_k)}^2\leq\epsilon$. % \footnote{In the following we will omit the subscript for the $\ell_2$-norm.} 
However, each iteration requires $N$ gradient computations, which can be too expensive for large values of $N$. Stochastic Gradient (SG) ascent~\citep[\eg][]{robbins1951stochastic,bottou2004large} overcomes this problem by sampling a single sample $x_i$ per iteration, but a vanishing step size is required to control the variance introduced by sampling. As a consequence, the lower per-iteration cost is paid with a worse convergence rate of $\mathcal{O}(\nicefrac{1}{\epsilon^2})$ \cite{nemirovskii1983problem}.
...
The corrected gradient $\blacktriangledown f(\vtheta)$ is an unbiased estimate of $\nabla f(\vtheta)$, and it is able to control the variance introduced by sampling even with a fixed step size, achieving a convergence rate of $O(\nicefrac{1}{\epsilon})$ without resorting to plain full gradient.

More recently, some extensions of variance reduction algorithms to the non-concave objectives have been proposed~\citep[\eg][]{reddi2016stochastic,allen2016variance,reddi2016stochastic,reddi2016fast}. In this scenario, $f$ is typically required to be L-smooth, \ie $\norm[2]{\nabla f(\vtheta') - \nabla f(\vtheta)} \leq L\norm[2]{\vtheta'-\vtheta}$ for each $\vtheta,\vtheta'\in\realspace^n$ and for some Lipschitz constant $L$. Under this hypothesis, the convergence rate of full gradient  and of stochastic gradient ascent are the same as in the concave case~\citep{nesterov2013introductory,ghadimi2013stochastic}. Also in this case, SVRG achieves the $O(\nicefrac{1}{\epsilon})$ rate \cite{reddi2016stochastic}. The only additional requirement is to select $\vtheta^*$ uniformly at random among all the $\vtheta_k$ instead of simply setting it to the final value. Pseudocode of SVRG for both the concave and the non-concave case is provided in Algorithm~\ref{alg:svrg}.
\todomat{check citations for convergence rates}
...
Once the last two terms can be neglected, the number of trajectories needed to achieve $\norm[2]{\gradJ{\vtheta}}^2\leq\epsilon$ is $O(\frac{B+\nicefrac{N}{m}}{\epsilon})$. Advantage over batch gradient ascent can be achieved with properly selected meta-parameters. In the following we propose practical ways to do so.
...
As we already mentioned in section \ref{sec:stopping} importance weights means a lot of variance to deal with. Therefore, in oreder to cope with this issue the literature propose the Self-normalization techniques \cite{precup2000eligibility}.
This technique offers a variance reduction over the importance weights in exchange of a bias introduction. Hence, if the bias introduced by this is smaller than the variance reduction it causes, it is convenient to use it in practice.  We expirienced some improvements using it (see section \ref{sec:exp} for the sperimental point of view).
Self-normalization is easy to use, but depends on the chosen reinforcement learning algorithm. Therefore, here we give the definition of this technique in the context of G(PO)MDP where we have used the weighted per-decision importance sampling estimator \cite{precup2000eligibility}:
\begin{align*}
...
\begin{equation}\label{E:offpolicygradient}
        \nabla J(\pi^T) = \EVV[\tau \sim \Delta(\cdot|\pi^B)]{\omega(\tau) \nabla \log \Delta(\tau|\pi^T) \mathcal{R}(\tau)}
\end{equation}
