...
In particular, the proposed SVRPG does not make explicit use of the structure of the policy gradient framework, and it is independent of the underlying gradient estimate (\ie w/o baseline).
...
\begin{figure*}[t]
	...
	\caption{On-line performance over sampled trajectories of SVRPG vs G(PO)MDP in the Cart-pole task, with 90\% t-student confidence intervals.}
	\label{fig:cartpole}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.32\textwidth}
...
Table \ref{tab:1} compares G(PO)MDP and SVRPG in terms of the average return computed on the whole learning process, which is a common metric for online learning since it is related to an average regret~\citep[\eg][]{duan2016benchmarking}. This metric corresponds to the area below the curves in the figures. Note that the convergence result in Theorem \ref{theo:convergence} is precisely about the average return. We also report normalized standard deviations and 90\% $t$-confidence intervals.
...
Despite that, we believe that it will be possible to derive a baseline designed explicitly for SVRPG to exploit the RL structure and the SVRG idea jointly. Future work should also focus on making batch sizes $N$ and $B$ adaptive, as suggested in \cite{papini2017adaptive}.
...
\begin{align*}
        \psi(s,t)=concat(s, s \circ s, 0.01t, (0.01t)^2, (0.01t)^3,1)
\end{align*}
where $s$ is the state vector, $\circ$ is the Hadamard (element-wise) product.
As a consequence, the baseline for both SVRPG and G(PO)MDP is defined as follows:
\[
        b(s,a,t) = \transpose{\boldsymbol{\lambda}} \psi(s,t)
\]
...
All the experiments' parameters are reported in Table~\ref{T:exp_settings}.

\begin{table}[b]
\centering
\caption{Settings of the experiments.}
\label{T:exp_settings}
...
\end{tabular}
\end{table}
In Figure~\ref{fig:cartpole_big},~\ref{fig:swimmertwo_big} and~\ref{fig:swimmerone_big} we have reported the same results presented in the main paper but on bigger figures.
...
\begin{figure}[t]
	...
	\includegraphics[width=.98\textwidth]{images/cart_pole_SVRPG_vs_GPOMDP_tex.pdf}
	\caption{On-line performance over sampled trajectories of SVRPG vs G(PO)MDP in the Cart-pole task, with 90\% t-student confidence intervals.}
	\label{fig:cartpole_big}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
	\includegraphics[width=0.98\textwidth]{swimmer_self_normalized_SVRPG_vs_SVRPG_tex.pdf}
	\caption{On-line performance over sampled trajectories of self-Normalized SVRPG vs unbiased SVRPG in the Swimmer task, with 90\% confidence intervals.}
	\label{fig:swimmertwo_big}
    \end{minipage}
\end{figure}
\begin{figure}[t]
        \centering
	\includegraphics[width=0.47\textwidth]{swimmer_self_normalized_SVRPG_vs_GPOMDP_tex.pdf}
	\caption{On-line performance over sampled trajectories of self-Normalized SVRPG vs G(PO)MDP in the Swimmer task, with 90\% confidence intervals}
	\label{fig:swimmerone_big}
\end{figure}
