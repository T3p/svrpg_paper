...
\[
\mathop{E}_{\tau_j\sim\pi(\cdot\mid\tilde{\vtheta}),\tau_i\sim\pi(\cdot\mid\vtheta)}
...
The parameter vector $\vtheta_A$ returned by Algorithm \ref{alg:svrpg} after $T=m\times S$ iterations has, for some constants $\psi>0, b>0$ and $c_{t}>0$ for $t=1,\dots,m$, the following property:
\[
	\EVV[]
...
\convergence*
\begin{proof}
We will prove the theorem for the following values of the open constants:
\begin{align*}
& \psi = \min_t\Psi_t, \\
& b = \max_t b_t.
& c_m = 0 \\
...
\begin{align*}
	...
	& b_t \coloneqq 3(\alpha_t\frac{L}{2} + c_{t+1}\alpha_t^2), \\
\end{align*}
