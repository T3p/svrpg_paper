...
%			\nabla f(\wt{\vtheta}^s) + 
	...
	\end{algorithmic}
\end{algorithm}
\todomat{Mention the fact that we return last parameter in practice}
...
As we already mentioned in section \ref{sec:stopping} importance weights means a lot of variance to deal with. Therefore, in order to cope with this issue the literature propose the Self-normalization techniques.
...
As regards our algorithm, the term
$
 \frac{1}{B} \sum_{i=0}^{B-1} \left[ \omega(\tau_i|\vtheta_t, \wt{\vtheta}) g(\tau_i|\wt{\vtheta})
\right]
$  \eqref{E:svrpg.estimate.batch} become:
\begin{align*}
\frac{1}{ \sum_{i=0}^{B-1}\omega(\tau_i|\vtheta_t, \wt{\vtheta})} \sum_{i=0}^{B-1} \left[ \omega(\tau_i|\vtheta_t, \wt{\vtheta}) g(\tau_i|\wt{\vtheta})
\right].
\end{align*}
The rest of terms remain unchanged.
We experienced some improvements using it (see section \ref{sec:exp} for the experimental point of view), in particular in our experiments we use this technique in the context of G(PO)MDP defined as "weighted per-decision importance sampling estimator" \cite{precup2000eligibility}.  

\subsection{Baseline}
As reported in Sec. \ref{subsec:PolicyGradient} a known method for variance reduction in Reinforcement Learning is the use of baseline.
This method is independent to SVRPO.
We decide to test SVRPO with an Actor Critic baseline: the baseline is a states dependent linear function with a time-varying feature vector \cite{duan2016benchmarking}.
Our choice is to use a single baseline for each term of expression \ref{E:svrpg.estimate.batch}. This is update only in the snapshot: the baseline is estimated using the N trajectories.
