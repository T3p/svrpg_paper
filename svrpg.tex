...
\end{align}
Notice that the distribution defining the gradient is induced by the current policy. This aspect introduces a nonstationarity in the sampling process. Since the underlying distribution is changing over time, it is necessary to resample at each update or use weighting techniques such as importance sampling.
...
the policy is updated as $\vtheta  = \vtheta + \alpha\gradApp{\vtheta}{N}$, where $\alpha$ is a step size and $\gradApp{\vtheta}{N}$ is an estimate of~\eqref{E:policygradient} using $N$ samples.
...
Under Assumption \ref{ass:bounded_score}, $J(\vtheta)$ is L-smooth.
\end{restatable}
