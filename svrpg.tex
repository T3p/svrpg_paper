...
For this reason, literature has focused on the introduction of baselines (\ie functions $b : \mathcal{S} \times \mathcal{A} \to \realspace$) aiming to reduce the variance~\citep[\eg][]{Peters2008reinf,Thomas2017actionbaseline,wu2018variance}.
...
In particular, the proposed SVRPG does not make explicit use of the structure of the policy gradient framework.
...
Additionally, the experiments show the advantage of using simple SVRPG against G(PO)MPD with baseline (see experiment of \textcolor{blue}{XXX} in Section~\ref{sec:exp}).
...
Task implementations are from the \textit{rllab} library \cite{duan2016benchmarking}\footnote{https://github.com/rll/rllab}, on which our agents are also based. More details on meta-parameters and exhaustive task descriptions are provided in Appendix \ref{app:exp}.
...
\begin{table*}[t]
	\caption{
		\label{tab:1}
		Comparison between G(PO)MDP and SVRPG on different tasks. Average return over the whole learning process is reported, together with normalized standard deviation and 90\% confidence intervals.}
	\centering
	\begin{tabular}{@{}lccccccc@{}} 
		...
		\\\cmidrule{2-8}
		Cart-pole & 618.49 & 21.09 & [570.76, 666.20] & \phantom{abc} 
		& \textbf{736.31} & 16.51 & [698.96, 773.65] \\ 
		\phantom{abc} & \phantom{abc} & \phantom{abc} & \phantom{abc} & \phantom{abc} & \phantom{abc} & \phantom{abc} \\
		Swimmer & 39.86 & 3.84 & [31.18, \:\:\: 48.53] & \phantom{abc} & 43.33 & 3.27 & [37.41, \:\:\: 49.25]\\
		\bottomrule
	\end{tabular}
\end{table*}

Figure \ref{fig:cartpole} compares SVRPG with G(PO)MDP on a continuous variant of the classical Cart-pole task, which is a 2D balancing task. Despite using more trajectories for each parameter, our algorithm shows faster convergence, which can be ascribed to the better quality of updates due to variance reduction.
...
Table \ref{tab:1} compares G(PO)MDP and SVRPG in terms of the average return computed on the whole learning process, which is a common metric for on-line learning~\citep[\eg][]{duan2016benchmarking}. This corresponds to the area below the curves in the figures. Note also that the convergence result Theorem \ref{theo:convergence} is precisely about average return. We also report normalized standard deviation and 90\% bootstrap confidence intervals.
In all the tasks, our algorithm has higher average return and lower normalized standard deviation. In the case of Cart-pole, disjoint confidence intervals show that the advantage is statistically significant. 
...
\end{align}
\endgroup
where (\ref{neweq:2}) is from (\ref{neweq:1}), (\ref{eq:1}) is from the definition of $\gradApp{\vtheta}{N}$, (\ref{eq:2}) is from (\ref{neweq:1}), (\ref{eq:3}) is from Lemma \ref{lemma:gsmooth}, 
...
\]
\eqref{eq:9} is from Young's inequality,~\eqref{eq:10} is from the definition of $\gradApp{\vtheta}{N}$, and \eqref{neweq:3} is from \eqref{neweq:1}.
\end{proof}
...
\begin{tabular}{| l | c  c  c |}
	...
	Learning rate SVRPO & $5\cdotp10^{-2}$ & $10^{-3}$ & $10^{-3}$ \\
	Learning rate GPOMDP & $10^{-2}$ & $10^{-3}$ & $10^{-3}$ \\
