...
\begin{align*}
\gradApp{\vtheta}{N} = \frac{1}{N}\sum_{n=1}^{N}\sum_{h=0}^{H-1}\left(\sum_{k=0}^{h}\score{\vtheta}{a_k^n\mid s_k^n}\right)\left(\gamma^h r_h^n - b\right).
\end{align*}
...
We introduce two stopping conditions: the maximum number of sub-iterations and the condition on learning rate of Adam sub-iterations.
