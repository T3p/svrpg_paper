...
\begin{align}
	...
	\frac{1}{2N}\Tr\Varts[]{\gradApp{\tilde{\vtheta}^s}{1}} -\frac{1}{2}\Ets[t-1]{\norm[]{f(\vtheta_t^{s+1})}^2},\nonumber
\end{align}
where (1) is from Definition \ref{def:ideal}; (2) is from the fact that $\gradIdeal{\vtheta_t^{s+1}}$ is both unbiased and independent from $f(\vtheta_t^{s+1})$ \wrt the sampling at time $t$ alone, which is not true for $\gradBlack{\vtheta_t^{s+1}}$; (3) is from $\Ets{e_s}=0$; (4) is from Young's inequality; and (5) is from the definition of $\gradApp{\vtheta}{N}$.
\end{proof}
...
\end{align}
where (6) is from the L-smoothness of $J(\vtheta)$, (7) is from the SVRPG update, and (8) is from Lemma \ref{lemma:aux0}.
...
\frac{\alpha_t}{N}\Tr\Varts[]{\gradApp{\tilde{\vtheta}^s}{1}} -\frac{1}{2}\Ets[t-1]{\norm[]{\vtheta_t^{s+1}-\tilde{\vtheta}^s}^2},
\end{align}
where (9) is from the SVRPG update, (10) is from Lemma \ref{lemma:aux0}, and (11) is from Young's inequality in the 'Peter-Paul' variant.

...[next: put (11),(8) and Lemma A.1 together to finish proof]
\end{proof}
