...
\begin{algorithm}[tb]
	...
	\label{alg:svrg}
	\begin{algorithmic}
		\STATE {\bfseries Input:} number of epochs $S$, epoch size $m$, step size $\alpha$, initial parameter $\vtheta_0$
		...
		\STATE $\blacktriangledown f(\vtheta_{s+1}^t) = 
					\nabla f(\tilde{\vtheta}) + 
					\nabla g(x_i\mid\vtheta_{s+1}^t) -
					\nabla g(x_i \mid \tilde{\vtheta})$
		\STATE $\vtheta_{s+1}^{t+1} = \vtheta_{s+1}^{t} + \alpha\blacktriangledown f(\vtheta_{s+1}^t)$
		\ENDFOR
		\STATE $\vtheta_{s+1} = \vtheta_{s+1}^m$
		\ENDFOR
		\STATE {\bfseries Convex case: return} $\vtheta_S$
		\STATE {\bfseries Non-Convex case: return} $\vtheta_s^t$ with $(s,t)$ picked uniformly at random from $[0,S)\times[0,m)$
	\end{algorithmic}
\end{algorithm}
...
\begin{align*}
	\blacktriangledown J(\vtheta) = &\frac{1}{N}\sum_{j=0}^{N-1}\nabla\log\pi(\tau_j \mid \tilde{\vtheta}) \\
		&+ \omega(\tau_i)\nabla\log\pi(\tau_i \mid \vtheta) 
		- \nabla\log\pi(\tau_i \mid \tilde{\vtheta}),
\end{align*}
where trajectories $\tau_j$ for $j=1,\dots,N$ and $\tau_i$ are all sampled independently from the snapshot policy $\pi_{\tilde{\vtheta}}$, and $\omega(\tau) = \frac{\pi(\tau\mid\vtheta)}{\pi(\tau\mid\tilde{\vtheta})}$ is an importance weight from $\pi_{\tilde{\vtheta}}$ to the current policy $\pol$. We immediately have the following (proof in the Appendix):
...
\]
\end{restatable}
The resulting SVRPG algorithm is detailed in [todo:pseudocode].
[Next: explain which details are left open and briefly outline next sections; deeper explanation of algorithm?]
