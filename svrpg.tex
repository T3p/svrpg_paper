...
\begin{algorithm}[tb]
	...
	\begin{algorithmic}
        ...
        \STATE Sample $N$ trajectories $\{\tau_j\}$ from $p(\cdot\vert\tilde{\vtheta}^{s})$
...
\begin{proof}
\begin{align*}
        \EVV[]{\gradBlack{\vtheta}} &= \EVV[]{\gradApp{\tilde{\vtheta}}{N}}  + \EVV[]{\gradApp{\vtheta}{B}} - \EVV[]{\frac{1}{B}\sum_{i=0}^{B-1}\omega(\tau_i|\vtheta, \wt{\vtheta}) g(\tau_i|\wt{\vtheta})} \\
...
+\frac{L_2^2}{B}\Ets[t-1]{\norm[2]{\vtheta_t^{s+1}-\tilde{\vtheta}^s}^2}
...
%            \Ets{\norm[2]{\gradBlack{\vtheta_t^{s+1}}}^2}
	...
	&= \mathbb{E}_{t\vert s}\left[\left\|\gradApp{\tilde{\vtheta}^s}{N}
			...
			% \right.\right.\nonumber
            % \\&\qquad\left.\left.
            \pm \gradJ{\vtheta_t^{s+1}} \pm \gradJ{\tilde{\vtheta}^s} 
...
%     \nonumber\\ 	&\qquad
			...
			\Ets{\norm[2]{
			g(\tau_i\vert\vtheta_t^{s+1}) -
    \omega(\tau_i)g(\tau_i\vert\tilde{\vtheta}^s) \pm g(\tau_i|\wt{\vtheta}^s) }^2} \nonumber\\
	%
%%	&= \Ets[t-1]{\norm[]{\gradJ{\vtheta_t^{s+1}}}^2} 
%%	+\frac{1}{N}\Vars{g(\cdot\vert\tilde{\vtheta}^s)}
%%	\nonumber\\
