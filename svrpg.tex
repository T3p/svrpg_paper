...
\begin{abstract}
        In this paper, we propose a stochastic variance-reduced version of policy gradient for solving Markov Decision Processes (MDPs).
...
For this reason, literature has focused on the introduction of baselines aiming to reduce the variance~\citep[\eg][]{Peters2008reinf,Thomas2017actionbaseline,wu2018variance}.\footnote{A baseline is a function $f(s,a)$ such that $\EVV[a \sim \pol]{f(s,a)} = 0$, for any $s\in\Sspace$. \todopir{check the definition of baseline}}
...
\end{description}
To deal with non-concavity, we require $J(\vtheta)$ to be $L$-smooth, which is a reasonable assumption for common policy classes such as truncated Gaussian and softmax~\citep[\eg][]{Furmston2012unifying,pirotta2015lipschitz}, see Appendix~\ref{app:gauss} for the derivation.
Because of the infinite dataset, we can only rely on an estimate of the full gradient.
\citet{harikandeh2015stopwasting} analysed this scenario under the assumptions of $z$ being \textcolor{red}{concave}, showing that SVRG is robust to an inexact computation of the full gradient. In particular, it is still possible to recover the original convergence rate if the error decreases at an appropriate rate. {\color{red}Later on we will show how the estimation accuracy impacts on the convergence results with non-concave objective.} 
...
We can now introduce Stochastic Variance-Reduced Policy Gradient (SVRPG) for a generic policy gradient estimator $g$ (see Eq.~\eqref{E:policygradient.estimate}).
...
% 		- \omega(\tau_i)\nabla\log\pi(\tau_i \vert \wt{\vtheta})\Reward(\tau_i),
% \end{align*}
where $\wh{\nabla}_N J(\wt{\vtheta})$ is as in Eq.~\eqref{E:policygradient.estimate} where $\mathcal{D}_N$ is sampled using the snapshot policy $\pi_{\wt{\vtheta}}$, $\tau_i$ is sampled from the current policy $\pi_{\vtheta_t}$, and $\omega(\tau|\vtheta_t, \wt{\vtheta}) = \frac{p(\tau|\wt{\vtheta})}{p(\tau|\vtheta_t)}$ is an importance weight from $\pi_{\vtheta_t}$ to the snapshot policy $\pi_{\wt{\vtheta}}$. 
...
\right]}_{c_t}.
\end{align}
We want just to stress that $\EVV[\tau_i \sim p(\cdot|\vtheta_t)]{\frac{1}{B} \sum_{i=0}^{B-1} \omega(\tau_i|\vtheta_t, \wt{\vtheta}) g(\tau_i|\wt{\vtheta})} = \nabla J(\wt{\vtheta})$.\footnote{The reader can refer to Appendix~\ref{} for off-policy gradients and variants of REINFORCE and G(PO)MDP.}
...
%\end{align*}
The use of mini-batches is a common practice in SVRG since it can yield a performance improvement even in the supervised case~\citep{harikandeh2015stopwasting,konevcny2016mini}. It is easy to show that the SVRPG estimator has the following, desirable properties (see Appendix~\ref{app:proofs}):
\begin{lemma}\label{L:svrpg.properties} \todopirout{extended to any unbiased estimator}
...
Algorithm~\ref{alg:svrpg} presents the details of the SVRPG method for policy optimization.\footnote{Note that when $t=0$ we can avoid to sample trajectories and instead directly perform the policy update using the FG. For the sake of clarity, we have not explicitly differentiated this step from the standard update rule in Algorithm~\ref{alg:svrpg}.}

To use SVRPG in practice, a number of additional details must be specified, namely the choice of epoch size $m$, the number $N$ of trajectories used to estimate the full gradient, the mini-batch size $B$, and the step size $\alpha$.
Adaptive selection of these meta-parameters is also possible, and it is explored in Section~\ref{sec:stopping}.
...
\begin{algorithm}[tb]
	...
	\label{alg:svrpg}
	\begin{algorithmic}
        \STATE {\bfseries Input:} number of epochs $S$, epoch size $m$, step size $\alpha$, batch size $N$, mini-batch size $B$, gradient estimator $g$, initial parameter $\vtheta_{0}^m := \wt{\vtheta}^0$
		\FOR{$s=0$ {\bfseries to} $S-1$}
        \STATE $\vtheta_0^{s+1} := \wt{\vtheta}^{s} = \vtheta_{m}^s$
...
%%		\end{align*}
		...
		\ENDFOR
		\ENDFOR
        \STATE {\bfseries return} $\vtheta_t^{s+1}$ with $(s,t)$ picked uniformly at random from $\{[0,S-1]\times[0,m-1]\}$
	\end{algorithmic}
\end{algorithm}

\section{Convergence Guarantees of SVRPG}\label{sec:conv}
In this section, we state the convergence guarantees for SVRPG {\color{red}with REINFORCE estimator}.\footnote{\color{red}Similar results can be derived for G(PO)MDP, see Appendix~\ref{app:}.}
...
In Appendix~\ref{app:gauss} we show how the traditional Gaussian policy satisfies these assumptions.
...
Similarly to what done in SL~\citep{cortes2010learning}, we require the variance of the importance weight to be bounded.
	\begin{restatable}[On the variance of importance weights]{assumption}{varweights}\label{ass:M2}
		...
		\]
	\end{restatable}
Differently from Assumptions~\ref{ass:bounded_score} and ~\ref{ass:REINFORCE}, Assumption~\ref{ass:M2} can be enforced by a proper handling of the epoch size $m$.

We can now state the convergence guarantees for SVRPG.
\begin{restatable}[Convergence of the {\color{red}REINFORCE-}SVRPG algorithm]{theorem}{convergence}\label{theo:convergence}
{\color{red}Consider the REINFORCE estimator in Eq.~\eqref{E:policygradient.estimate}.}
...
Once the last two terms can be neglected, the number of trajectories needed to achieve $\norm[2]{\gradJ{\vtheta}}^2\leq\epsilon$ is $O(\frac{B+\nicefrac{N}{m}}{\epsilon})$. In this sense, advantage over batch gradient ascent can be achieved with properly selected meta-parameters. In Section~\ref{sec:stopping} we propose a joint selection of step size $\alpha$ and epoch size $m$.
...
The step size $\alpha$ is crucial to balance the variance due to the sub-iterations and the variance associated with the estimator at the snapshot.
...
The number $m$ of sub-iterations influences the variance introduced by the importance weights. Low $m$ values are associated with low variance but induce lots of snapshot points (\ie lots of FG computations). High $m$ values may move policy $\pi_{\vtheta_t}$ far away from the snapshot policy $\pi_{\wt{\vtheta}}$, causing high variance of the importance weights.
...
\end{align*}
where $\alpha^{\textsc{FG}}_{s}$ is associated to the snapshot and $\alpha^{\textsc{SI}}_{s,t}$ with the sub-iterations (see Appendix~\ref{app:practicalsvrpg} for details).
...
As mentioned in Section~\ref{sec:stopping}, importance weights are an additional source of variance. A standard way to cope with this issue is self-normalization.
...
\right].
\end{align*}
In Section \ref{sec:exp} we show some cases in which self-normalization provides a performance improvement.
\todomat{add reference, maybe just Precup}

\subsection{Baseline}
As reported in Section~\ref{subsec:PolicyGradient}, a popular variance reduction tool in Reinforcement Learning is the employment of baselines in policy gradient estimation.
This tool is orthogonal to the variance reduction technique proposed in this paper. In Section~\ref{sec:exp} we try to combine SVRPG with a non-stationary, state-dependent, learned linear baseline like the one used in~\cite{duan2016benchmarking}, with the variant that baseline parameters are updated only at the snapshot, with trajectories sampled from $\pi_{\wt{\vtheta}^s}$.
...
In this section we address the performance evaluation of our policy gradient estimator defined in equation \ref{E:svrpg.estimate.batch} against that one of equation \ref{E:policygradient.estimate}. This comparison is performed using G(PO)MDP as estimator of $\score{\vtheta}{\tau}\Reward(\tau)$. For the sake of simplicity and with a little abuse of notation we refer to \ref{E:svrpg.estimate.batch} as SVRPG and to \ref{E:policygradient.estimate} as G(PO)MDP.
In all the experiments we used gaussian policies and bootsrap confidence intervals $\alpha = 10\% $ over ten different runs for evaluation. Each run with a different policy initialization which is kept fixed when changing the algorithm. The maximum number of trajectories at hand is fixed for each task. In \emph{figure} \ref{fig:swimmerone} we can see how SVRPG outperforms G(PO)MDP over the swimmer task. Clearly the mean is nearly always higher for our algorithm, moreover near the end we see that te confidence intervals are also disjoint. In \emph{figure} \ref{fig:swimmertwo} we addressed the comparison between SVRPG and its self-normalized version still for the swimmer task. We can see an inprovement on the average return, but the confidence intervals are never disjoint. In \emph{figure} \ref{fig:halfcheetah} the difference between our algorithm and G(PO)MDP over the half-cheetah task is not statistically significant (both of them use the same baseline even though is not optimal for SVRPG see \ref{} for further hints over this topic), however we can see a certain improvement on the mean of the objective function. Eventually in \emph{figure} \ref{fig:cartpole} we can clearly see that SVRPG outperforms GPOMDP in the cart-pole task.

\begin{figure}[t]
    \includegraphics[width=0.5\textwidth]{swimmer_self_normalized_SVRPG_vs_GPOMDP}
    \caption{Self-Normalized SVRPG vs GPOMDP in swimmer}
    \label{fig:swimmerone}
\end{figure}
\begin{figure}[t]
    \includegraphics[width=0.5\textwidth]{swimmer_self_normalized_SVRPG_vs_SVRPG}
    \caption{Self-Normalized SVRPG vs Non Self-Normalized SVRPG in swimmer}
    \label{fig:swimmertwo}
\end{figure}
\begin{figure}[t]
    \includegraphics[width=0.5\textwidth]{half_cheetah_self_norm_baseline_SVRPG_vs_GPOMDP100_baseline}
    \caption{Self-Normalized SVRPG with baseline vs GPOMDP with baseline in half cheetah}
    \label{fig:halfcheetah}
\end{figure}
\begin{figure}[t]
    \includegraphics[width=0.5\textwidth]{cart_pole_SVRPG_vs_GPOMDP}
    \caption{SVRPG vs GPOMDP in cart-pole}
    \label{fig:cartpole}
\end{figure}
...
Finally, we have empirically shown that practical variants of the theoretical SVRPG version can outperform classical actor-only approaches on several benchmarks.
...
}_{g(\tau_n|\vtheta):=\nabla \log\Delta(\tau_n|\vtheta)\Reward(\tau_n)}
,
\end{align*}
...
\]
where $\omega(\tau|\pi^B,\pi^T) = \frac{\Delta(\tau,\pi^T)}{\Delta(\tau|\pi^B)} = \omega(z_{0:H}|\pi^B,\pi^T) = \prod_{w=0}^{H} \omega(z_w|\pi^B,\pi^T)$ and $\omega(z_w|\pi^B,\pi^T) = \frac{\pi^T(a_w|s_w)}{\pi^B(a_w|s_w)}$
...
\begin{equation}\label{E:offpolicygradient}
        \nabla J(\pi^T) = \EVV[\tau \sim \Delta(\cdot|\pi^B)]{\omega(\tau|\pi^B,\pi^T) \nabla \log \Delta(\tau|\pi^T) \mathcal{R}(\tau)} = \EVV[\tau \sim \Delta(\cdot|\pi^B)]{\omega(\tau|\pi^B,\pi^T) g(\tau|\pi^T)}. 
\end{equation}
For $\omega(\tau)$ being well defined the behavioural policy should have non-zero probability of selecting any action in every state \ie $\pi^B(a|s) > 0$ for any $(s,a)\in \mathcal{S} \times \mathcal{A}$.
...
\[
        \nabla J(\pi^T) = \frac{1}{N} \sum_{n=1}^{N} \omega(\tau^n)
        \left(\sum_{h=0}^{H}\score{\vtheta}{a_h^n\vert s_h^n}\right)\left(\sum_{h=0}^{H}\gamma^h r(s_h^n,a_h^n)\right).
\]

The G(PO)MDP off-policy estimator is defined as follows
\[
        \nabla J(\pi^T) = \frac{1}{N} \sum_{n=1}^{N}
        \sum_{h=0}^H \left(\sum_{k=0}^h \nabla \log\pi^T(z_k)\right) \gamma^h r(z_h^n) \omega(z_{0:h})
\]
...
\begin{align*}
		...
		\mathop{\mathbb{E}}_{\substack{\tau_j\sim\pi(\cdot\vert\wt{\vtheta}^s)\forall j \in {\it {\bf N}} \\ \tau_{i,h}\sim\pi(\cdot\vert\vtheta^{s+1}_i) \forall h \in {\it {\bf B}}, \text{ for $i=0,\dots,t-1$}}}{\left[X \vert \wt{\vtheta^s}\right]} \\
...
\end{align*}
where the sequence $\wt{\vtheta}^s,\vtheta_0^{s+1},\dots,\vtheta_t^{s+1}$ is defined in Algorithm \ref{alg:svrpg}, ${\it {\bf N}} = [0,\dots,N)$, and ${\it {\bf B}} = [0,\dots,B)$. 
\todopir{what about $\vtheta^{s+1}_0$ and $\wt{\vtheta}^s$? They are equal. How this affect previous definition?}
...
\[
	\norm[2]{g(\tau\vert\vtheta)-g(\tau\vert\vtheta')}^2 \leq L_2\norm[2]{\vtheta-\vtheta'}^2.
\]
\todopir{I would denote $L_J$ and $L_g$ rather than 1 and 2}
\end{restatable}
\begin{proof}
...
\end{align*}
For G(PO)MDP, we do not have a compact expression for $g$, but since it is derived from REINFORCE by neglecting some terms of the kind $\nabla \log \pi_{\vtheta}{a_t | s_t}\gamma^t r_t$ \cite{baxter2001infinite,peters2008reinforcement}, the above bound still holds. \todopir{we need to prove it formally if we have time}
\end{proof}
%Lemma 2
...
\end{align*}
\todopir{I have to check this one!!!}
\end{restatable}
\begin{proof}
\begin{align}
...
\begin{align}
	...
	\nonumber\\
	&\leq
	\sqrt{\Vars{\gradApp{\wt{\vtheta}^s}{N}}}\circ\sqrt{\Varts[t-1]{\varphi(\vtheta_t^{s+1})}} \nonumber\\
	%
	&\leq	
...
(\ref{eq:new1}) is from the fact that $\gradJ{\wt{\vtheta}^s}$ is constant \wrt $\Vars{\cdot}$;
...
\end{align}
where~\eqref{eq:11} is from the L-smoothness of $J(\vtheta)$ and \eqref{eq:12} is from the SVRPG update.
{\color{blue} Inequality~\ref{eq:13} follows from Lemma~\ref{lemma:aux0} by noticing that $\nabla J(\vtheta^{s+1}_t)$ is a deterministic function given $\vtheta^{s+1}_t$. As a consequence we can directly apply Lemma~\ref{lemma:aux0} to lower-bound $\EVV[t|s]{\langle \nabla J(\vtheta^{s+1}_t, \blacktriangledown J(\vtheta^{s+1}_t \rangle}$ by $\norm[2]{\nabla J(\vtheta^{s+1}_t}^2 - f(\vtheta^{s+1}_t, \wt{\vtheta}^s)$.}
\todopir{is \eqref{eq:11} true just for smooth functions? Does it require convexity/concavity? Ref?}
...
\allowdisplaybreaks
\begin{align}
\Ets{\norm[]{\vtheta_{t+1}^{s+1}-\wt{\vtheta}^s}^2} 
...
%
&\leq \Ets{\alpha_t^2\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2+\norm[]{\vtheta_t^{s+1}-\wt{\vtheta}^s}^2}
+2\alpha_t\Ets{\left|\dotprod{\gradJ{\vtheta_t^{s+1}}}{\vtheta_t^{s+1}-\wt{\vtheta}^s}\right|} \nonumber\\ 
&\qquad+
\frac{\alpha_t}{N}\Vars{g(\cdot\vert\wt{\vtheta}^s)} +\alpha_t\Ets[t-1]{\norm[]{\vtheta_t^{s+1}-\wt{\vtheta}^s}^2} \nonumber\\
%
&\leq \Ets{\alpha_t^2\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2+\norm[]{\vtheta_t^{s+1}-\wt{\vtheta}^s}^2}
...
\end{align}
\endgroup
where~\eqref{eq:14a} is obtained using the triangular inequality,~\eqref{eq:14} is from the SVRPG update,~\eqref{eq:15} is from Lemma~\ref{lemma:aux0},~\eqref{eq:16a} is from Cauchy-Schwarz inequality, and~\eqref{eq:16} is from Young's inequality in the `Peter-Paul' variant.
Finally:
\begingroup
\allowdisplaybreaks
\begin{align}
