...
\begin{align}\label{E:svrpg.estimate.batch}
        ...
        \right].
\end{align}
We want just to underlying that $\EVV[\tau_i \sim \pi_{\vtheta_t}]{\frac{1}{B} \sum_{i=0}^{B-1} \omega(\tau_i|\vtheta_t, \wt{\vtheta}) g(\tau_i|\wt{\vtheta})} = \nabla J(\wt{\vtheta})$~\citep[\eg][]{jurvcivcek2012reinforcement}.\footnote{The reader can refer to App.~\ref{} for off-policy variants of REINFORCE and G(PO)MDP.}
...
However, the next section will provide general converge guarantees for SVRPG with REINFORCE estimator.
...
In this section, we state the convergence guarantees for SVRPG with REINFORCE estimator (refer to Alg.~\ref{alg:svrpg} with $g(\tau|\vtheta) =  \nabla \log\pi(\tau|\vtheta) \mathcal{R}(\tau)$).\footnote{Similar results can be derived for G(PO)MDP, see App.~\ref{app:}.}
...
As we already mentioned in section \ref{sec:stopping} importance weights means a lot of variance to deal with. Therefore, in oreder to cope with this issue the literature propose two main tecniques:
\begin{enumerate}
\item Self-normalization (?ciatazione bib?).
\item Importance weights trucation (?citazione bib?).
\end{enumerate}
Both of them offer a variance reduction over the importance weights in exchange of a bias introduction. Hence, if the bias introduced by those techniques is smaller than the variance reduction they cause, it is convenient to use them in practice. So far, sperimentally, we have not seen any advantage in using importance weights truncation, hence we discard this technique for the previously mentioned purpose. Whereas for what concern self-normalization we expirienced some improvements using it (see section \ref{sec:exp} for the sperimental point of view).
