...
\end{align*}
This kind of optimization is very common in machine learning, where each $g_i$ may correspond to a data sample $x_i$. In this case, we adopt the following, more meaningful notation:
\begin{align*}
\max_{\vtheta} f(\vtheta) = \frac{1}{N}\sum_{i=1}^{N}g_{\vtheta}(x_i).
\end{align*}
A common requirement is that $g$ must be smooth and convex in $\vtheta$. Under this hypothesis, full gradient ascent with a constant step size achieves a convergence rate of $O(\nicefrac{1}{k})$, \ie $f(\vtheta^*) - f(\vtheta_k) = O(\nicefrac{1}{k})$, where $\vtheta^*$ is the optimal parameter and $\vtheta_k$ is the parameter after $k$ gradient updates. The problem with full gradient is that each update can be very expensive, especially when $N$ is large. Stochastic gradient ascent overcomes this problem, at the cost introducing some variance. As a result, the (expected) convergence rate drops to $O(\nicefrac{1}{\sqrt{k}})$.
