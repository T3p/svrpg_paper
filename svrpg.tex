...
\end{align*}
In the rest of this section we assume this practical variant.
...
\end{align*}
where $\alpha^{\textsc{FG}}_{s}$ is associated to the snapshot and $\alpha^{\textsc{SI}}_{s+1,t}$ with the sub-iterations (see Appendix~\ref{app:practicalsvrpg} for details).
...
\todopir{In Eq~\ref{eq:7} the first expected value is on $t-1$? In any case even $t$ is fine.}
Hence:
\begin{align}
...
\]
\eqref{eq:9} is from Young's inequality, and~\eqref{eq:10} is from the definition of $\gradApp{\vtheta}{N}$.
\todopir{comment the last step.}
\end{proof}
