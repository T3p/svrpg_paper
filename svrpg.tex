...
In order to cope with this behaviour we introduce an adaptive stopping condition: we permorm a new snapshot anytime the learning rate proposed by the adam optimizer in a sub-iteration is 10 times smaller than that one proposed at the immediately previous snapshot. This stopping condition is subject to a constraint on the maximum number of sub-iterations performable, which is problem dependent.
