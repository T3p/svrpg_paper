...
\todopir{This definition of episodic task is not precise!
        An episodic task is a task in which there is an absorbing state (at least one) that is optimal. This means that the optimal policy reaches the target state in a finite number of steps. A part from that, there is no other property on the problem. The previous sentences may be misinterpreted and can lead to think that the problem is finite horizon but in that case we know that the optimal policy is non-stationary (bad)!
...
\end{align*}
Solving an MDP means finding a policy $\pi^* \in \argmax_{\pi} \{J(\pi)\}$.
...
\[
\mathop{\mathbb{E}}
\left[\blacktriangledown J(\vtheta)\right] = \gradJ{}.
\]
\end{restatable}
...
\begin{proof}
	...
	&\geq \Ets{J(\vtheta_t^{s+1})+\alpha_t\dotprod{\gradJ{\vtheta_t^{s+1}}}{\gradIdeal{\vtheta_t^{s+1}}+e_s} - \frac{\alpha_t^2L}{2}\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2} \\
	&= \Ets{J(\vtheta_t^{s+1})+\alpha_t\norm[]{\gradJ{\vtheta_t^{s+1}}^2} - \frac{\alpha_t^2L}{2}\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2}  + \alpha_t\Ets[t-1]{\dotprod{e_s}{\gradJ{\vtheta_t^{s+1}}}} \nonumber\\
	&= \Ets{J(\vtheta_t^{s+1})+\alpha_t\norm[]{\gradJ{\vtheta_t^{s+1}}^2} - \frac{\alpha_t^2L}{2}\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2} \\
	&\qquad+
	\alpha_t\Tr\Covts[t-1]{\gradApp{\tilde{\vtheta}^s}{N}}{\gradJ{\vtheta_t^{s+1}}} \\
	&\geq  \Ets{J(\vtheta_t^{s+1})+\alpha_t\norm[]{\gradJ{\vtheta_t^{s+1}}^2} - \frac{\alpha_t^2L}{2}\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2} \nonumber\\
	&\qquad-
	\alpha_t\Tr\left|\Covts[t-1]{\gradApp{\tilde{\vtheta}^s}{N}}{\gradJ{\vtheta_t^{s+1}}}\right| \nonumber \\
	%
	&\geq  \Ets{J(\vtheta_t^{s+1})+\alpha_t\norm[]{\gradJ{\vtheta_t^{s+1}}^2} - \frac{\alpha_t^2L}{2}\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2} \nonumber\\
	&\qquad-\alpha_t \Tr\left(\sqrt{\Varts[t-1]{\gradApp{\tilde{\vtheta}^s}{N}}}\circ\sqrt{\Varts[t-1]{\gradJ{\vtheta_t^{s+1}}}}\right) \nonumber \\
	%
	&\geq  \Ets{J(\vtheta_t^{s+1})+\alpha_t\norm[]{\gradJ{\vtheta_t^{s+1}}^2} - \frac{\alpha_t^2L}{2}\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2} \nonumber\\
	&\qquad-\frac{\alpha_t}{N} \Tr\left(\sqrt{\Varts[t-1]{\gradApp{\tilde{\vtheta}^s}{1}}}\circ\sqrt{\Varts[t-1]{\gradJ{\vtheta_t^{s+1}}}}\right) \\
	%
	&=
	\Ets{J(\vtheta_t^{s+1})+\alpha_t\norm[]{\gradJ{\vtheta_t^{s+1}}^2} - \frac{\alpha_t^2L}{2}\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2} \nonumber\\
	&\qquad-\frac{\alpha_t}{2N}\left(\Tr\Varts[t-1]{\gradApp{\tilde{\vtheta}^s}{1}} + \Tr\Varts[t-1]{\gradJ{\vtheta_t^{s+1}}}\right) \\
	%
	&\geq  \Ets{J(\vtheta_t^{s+1})+\alpha_t\norm[]{\gradJ{\vtheta_t^{s+1}}^2} - \frac{\alpha_t^2L}{2}\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2} \nonumber\\
	&\qquad-\frac{\alpha_t}{2N}\left(\Tr\Varts[t-1]{\gradApp{\tilde{\vtheta}^s}{1}} + \Ets[t-1]{\norm[]{\gradJ{\vtheta_t^{s+1} - \gradJ{\tilde{\vtheta}^s}}}^2}\right) \nonumber\\
	%
	&\geq  \Ets{J(\vtheta_t^{s+1})+\alpha_t\norm[]{\gradJ{\vtheta_t^{s+1}}^2} - \frac{\alpha_t^2L}{2}\norm[]{\gradBlack{\vtheta_t^{s+1}}}^2} \nonumber\\
	&\qquad-\frac{\alpha_tL^2}{2N}\left(\Tr\Varts[t-1]{\gradApp{\tilde{\vtheta}^s}{1}} + \Ets[t-1]{\norm[]{\vtheta_t^{s+1} - \tilde{\vtheta}^s}^2}\right)
\end{align}
where (1) is from the L-smoothness of $J(\vtheta)$, (2) is from the SVRPG gradient ascent update, (3) is from Definition \ref{def:ideal}, (4) is from the fact that $\EVV[\tau_t\sim\pi(\cdot\mid \vtheta_t^{s+1})]{\gradBlack{\vtheta_t^{s+1}}\mid\vtheta_t^{s+1}} = \EVV[\tau_t\sim\pi(\cdot\mid \vtheta_t^{s+1})]{\gradIdeal{\vtheta_t^{s+1}}\mid\vtheta_t^{s+1}}$, (5) is from the fact that $\Ets[t-1]{e_s} = 0$, (6) is from the definition of $\gradApp{\vtheta}{N}$, (7) is from Young's inequality and (8) is again from L-smoothness.
