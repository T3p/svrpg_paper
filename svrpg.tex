...
Policy gradient methods restrict the search for the performance-maximizing policy over a class of parametrized policies $\Pi_{\vtheta}=\{\pol: \vtheta \in \realspace^d\}$, with the only constraint that $\pol$ is differentiable \wrt $\vtheta$. We denote the performance of a parametric policy with $J(\vtheta)$ for brevity, while in some occasions we replace $p_{\vtheta}(\tau)$ with $p(\tau|\vtheta)$ for the sake of readability.
...
\end{align}
where $g(\tau_i|\vtheta)$ is an estimate of $\score{\vtheta}{\tau_i}\Reward(\tau_i)$, \eg REINFORCE~\citep{williams1992simple} and G(PO)MDP~\citep{baxter2001infinite}.
...
As mentioned in section \ref{sec:stopping}, importance weights are an additional source of variance. A common way to cope with this issue is self-normalization.
...
\subsection{Baseline}
As reported in Sec. \ref{subsec:PolicyGradient}, a well known variance reduction tool in Reinforcement Learning is the employment of baselines in policy gradient estimation.
