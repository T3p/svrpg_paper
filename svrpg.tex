...
We use two different adam hyperparameters: one for updates tied to snapshot points and the other tied to the updates of sub-iterations points.
\todomat{$\uparrow$ explain better: it's not only two separate meta-parameters, but two separate learning rate schedules. Use notation from previous sections, \eg snaphot is usually associated with $\tilde{\vtheta}$} 
This is because we are dealing with two different variance magnitudes.
\todomat{$\uparrow$ the reason is this, but try to spend more words on it}
...
In order to cope with this behaviour we introduce an adaptive stopping condition: we perform a new snapshot anytime the weighted learning rate proposed by the adam optimizer in a sub-iteration is smaller than the weighted one proposed at the immediately previous snapshot. This stopping condition is subject to a constraint on the maximum number of sub-iterations performable, which is problem dependent.
\todomat{I find "two stopping conditions" confusing: it's just one with an additional constraint}
\todobep{I tried to solve the issue here:hope it's better now}
We stop with inner iterations if:

\[\frac{\alpha_{snap}}{N}>\frac{\alpha_{sub-iter}}{B}\]

\todomat{why? should mention how this formula combines trajectory efficiency and variance control}
After that we sample N trajectories for the next Snapshot point.


\section{Practical Variants}\label{sec:prac}
\todomat{Missing opening to section}
As reported in Section \ref{subsec:PolicyGradient} we use the G(PO)MDP gradient estimator~\cite{baxter2001infinite}, which gives us the same guarantees we have for the Reinforce Estimator.
