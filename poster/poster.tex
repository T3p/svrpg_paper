...
\newcommand{\hlo}[1]{\hl{orange!50}{#1}}
\newcommand{\hly}[1]{\hl{yellow!70!black!50!white}{#1}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \noindent \hly{Sample efficiency} is crucial in Reinforcement Learning (RL), where collecting samples is extremely expensive\\ 
    \hspace*{.3cm} $\implies$ \textbf{FG} often \underline{unfeasible}

	\vspace{2mm}
	\noindent Slower convergence of \textbf{SG} due to high \hly{gradient variance}.
    \end{minipage}
	
    
    \vspace{.3cm}
    \noindent $\blacktriangleright$~ \hlr{Solution:} from finite-sum optimization in Supervise Learning:
	\begin{itemize}\compresslist
...
\noindent\begin{minipage}[t]{.32\textwidth}
        \hlg{The easy part:}\\[.2cm]
%\begin{itemize}\compresslist
        $\bullet$~Take any \textbf{unbiased} policy gradient estimator $g(\tau|\vtheta)$ (\eg REINFORCE)\\[.15cm]
    $\bullet$~\textbf{SVRG} idea: combine \hly{frequent \textbf{SG}} with \hly{rare \textbf{FG}} into a corrected gradient estimator $\gradBlack{\vtheta}$\\[.15cm]
    $\bullet$~Two-time scales:\\ \hspace*{.8cm} $1$ \textbf{epoch} $=m$ \textbf{iterations}
%\end{itemize}
\end{minipage}
\hfill%
\begin{minipage}[t]{.33\textwidth}
        \hlo{Solving the RL challenges:}\\[.2cm]
...
\begin{minipage}[t]{.31\textwidth}
        \vspace*{-.4cm}
        \hspace*{1.8cm} \emph{Algorithm}
        \vspace*{-.2cm}
        \bookboxx{
                \small
                \textbf{For} $s = 1,\ldots$\\
                \hspace*{.2cm} Sample $N$ trajectories using $\theta^s$\\
                \hspace*{.2cm} Compute FG $\wh{\nabla}_N J(\theta^S)$\\
                \hspace*{.2cm} \textbf{For} $t = 1, \ldots, m-1$\\
                \hspace*{.4cm} Sample $B$ trajectories using $\theta^{s+1}_t$\\
                \hspace*{.4cm} Compute SG $\wh{\nabla}_B J(\theta^{s+1}_t)$ and \\
                \hspace*{.6cm} $c_t^{s+1} = \omega(\theta^{s+1}_t, \theta^s)\wh{\nabla}_B J(\theta^{s+1}_t)$\\
                \hspace*{.4cm} Compute $\theta^{s+1}_{t+1}$ using $\gradBlack{\theta^{s+1}_t}$
        }
\end{minipage}
...
\begin{tabu} to \textwidth{X[c]X[2c]X[c]}
    ...
    \tikz[baseline]{\node[anchor=base, fill=yellow](C){$\omega(\vtheta, \wt{\vtheta}) \wh{\nabla}_B J({\wt{\vtheta}})$};}$}
		};
	}  
	\vspace{0.9cm} 
	& \multirow{2}{*}{
		\shortstack{
            \tikz[baseline]{\node[anchor=base, fill=yellow, inner sep=2pt](Ctext){$\textbf{Correction term}$};}\\
...
\tikz[overlay]{
        ...
        \path[->, yellow, thick] (C.east) edge[bend left] (Ctext.west);
...
\begin{minipage}[t]{.55\textwidth}
	Fundamental \textbf{SVRG} properties:
	\begin{enumerate}
...
\begin{minipage}[t]{.4\textwidth}
        \hly{\textbf{Importance weighting}}
