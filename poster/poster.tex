...
%\begin{itemize}\compresslist
    ...
    $\bullet$~\textbf{SVRG} idea: combine \hlr{frequent \textbf{SG}} with \\\hlg{rare \textbf{FG}} into a \hly{corrected} gradient estimator $\gradBlack{\vtheta}$\\[.15cm]
...
\begin{center}
        Introducing the \large{\enhance{SVRPG}} estimator $\gradBlack{\vtheta}$:
\end{center}
...
\begin{minipage}[t]{.48\textwidth}
	...
	\begin{enumerate}
	\item \textbf{Unbiasedness:} $\EVV[]{\gradBlack{\vtheta}} = \gradJ{\vtheta}$
	\item \textbf{Vanishing variance:} $\Var{\gradBlack{\vtheta}}\to0\:\:$ as $\:\:\vtheta\to\vtheta^*$
	\end{enumerate}
	\end{tcolorbox}
\end{minipage}
\hfill%
\begin{minipage}[t]{.48\textwidth}
\begin{tcolorbox}[left=0cm,right=0cm,top=0cm,bottom=0cm,colback=white, boxsep=.1cm, colframe=yellowp, grow to left by=0cm, grow to right by=0cm]
\vspace*{-.11cm}\hspace*{-.2cm}
...
\begin{itemize}\compresslist
\item Constants ($\psi,\zeta,\xi$) depend only on \hlg{step size $\alpha$} and \hly{epoch size $m$}
...
Heuristics can improve performance in practice
	\begin{itemize}
		\item \hlg{Step size:} \textbf{ADAM} to counteract gradient variance\\
		...
		\item \hly{Epoch size:} cut epoch when the effective step size becomes too small
		\begin{equation*}
		...
		\item \hlr{Critic:} an orthogonal variance reduction technique from the \textbf{PG} literature
	\end{itemize}
}
...
\noindent
\begin{tikzpicture}
	\node[inner sep=0pt] (picture1) {\includegraphics[width=.98\textwidth]{images/cartpole.pdf}};
	\node[above left] at ($(picture1.north west) + (2.7cm,-.6cm)$) {\textbf{Cart-Pole}};
\end{tikzpicture}%

\noindent
\begin{tikzpicture}
\node[inner sep=0pt] (picture2) {\includegraphics[width=0.98\textwidth]{images/swimmer.pdf}};
...
\noindent
\begin{tikzpicture}
\node[inner sep=0pt] (picture3) {\includegraphics[width=0.98\textwidth]{images/cheetah.pdf}};
